...
if (!$res->fetch_assoc()) {
    $mysqli->query("INSERT INTO users (username,password,role) VALUES ('admin','admin123','admin')");
    echo 'Created admin (admin/admin123)<br>';
}

$res = $mysqli->query("SELECT id FROM workers WHERE name='Worker One' LIMIT 1");
if (!$res->fetch_assoc()) {
    $mysqli->query("INSERT INTO workers (name,password,field,salary) VALUES ('Worker One','worker123','Network',3000)");
    $wid = $mysqli->insert_id;
    $mysqli->query("INSERT INTO users (username,password,role) VALUES ('worker$wid','worker123','worker')");
    echo "Created worker, login username: worker$wid password worker123<br>";
}

$res = $mysqli->query("SELECT id FROM customers WHERE name='Customer One' LIMIT 1");
if (!$res->fetch_assoc()) {
    $mysqli->query("INSERT INTO customers (name,password,activated_package) VALUES ('Customer One','cust123','Basic')");
    $cid = $mysqli->insert_id;
    $mysqli->query("INSERT INTO users (username,password,role) VALUES ('customer$cid','cust123','customer')");
    echo "Created customer, login username: customer$cid password cust123<br>";
}
